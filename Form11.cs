...
namespace VEERA.ADO
{
    public partial class Form11 : Form
    {
        ...
        private void textBox6_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select * from sdb where SoilName like '" + this.textBox6.Text + "%'", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                dataGridView1.DataSource = dt;
                ...
                if (dt.Rows.Count == 1)
                {
                    ...
                    textBox6.Text = row["SoilName"].ToString();
                    textBox5.Text = row["SoilType"].ToString();
                    textBox4.Text = row["pHValue"].ToString();
                    textBox3.Text = row["MoistureContent"].ToString();
                    textBox2.Text = row["NitrogenContent"].ToString();
                    textBox1.Text = row["GrowingPlants"].ToString();
                }
